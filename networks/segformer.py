...
class Scale_reduce(nn.Module):
    def __init__(self, dim, reduction_ratio, img_size):
        ...
        if len(self.reduction_ratio) == 4:
            ...
            self.sr1 = nn.Conv2d(
                dim * 2, dim * 2, reduction_ratio[2], reduction_ratio[2]
            )
            self.sr2 = nn.Conv2d(
                dim * 5, dim * 5, reduction_ratio[1], reduction_ratio[1]
            )
...
class LocalEnhance_EfficientSelfAtten(nn.Module):
    ...
    def forward(self, x: torch.Tensor, H, W) -> torch.Tensor:
        ...
        local_pos = (
            self.local_pos(local_v)
            .reshape(B, -1, self.head, C // self.head)
            .permute(0, 2, 1, 3)
        )
...
class MixD_FFN(nn.Module):
    ...
    def forward(self, x):
        ...
        fuse = (
            self.act(ax + self.fc1(x))
            if self.fuse_mode == "add"
            else self.act(torch.cat([ax, self.fc1(x)], 2))
        )
        out = self.fc2(ax)
        return out
...
class DCPool(nn.Module):
    def __init__(
        ...
        self.maxpool = nn.Sequential(
            nn.MaxPool2d(kernel_size=ksize, stride=stride, padding=padding),
        ...
        self.xpool = nn.Sequential(
            nn.AdaptiveAvgPool2d([patch, 1]), nn.ReLU(inplace=True)
        )

        self.ypool = nn.Sequential(
            nn.AdaptiveAvgPool2d([1, patch]), nn.ReLU(inplace=True)
        )
        ...
        if self.mode == "LC":
            self.project = nn.Conv2d(
                in_channels=3 * inc, out_channels=outc, kernel_size=1
            )
            self.conv = nn.Conv2d(
                in_channels=outc, out_channels=outc, kernel_size=1, stride=1
            )
            self.righconv = nn.Conv2d(
                in_channels=inc,
                out_channels=outc,
                kernel_size=ksize,
                stride=stride,
    ...
    def forward(self, x):
        if self.mode == "DC":
            ...
            raise NotImplementedError(
                "NO exist mode! Please check mode whether use correct!!!"
            )
...
class OverlapPatchEmbeddings(nn.Module):
    def __init__(
        self, img_size=224, patch_size=7, stride=4, padding=1, in_ch=3, dim=768
    ):
        super().__init__()
...
class MiT(nn.Module):
    def __init__(self, image_size, dims, layers, token_mlp="mix_skip"):
        ...
        self.patch_embed1 = OverlapPatchEmbeddings(
            image_size, patch_sizes[0], strides[0], padding_sizes[0], 3, dims[0]
        )
...
class Decoder(nn.Module):
    ...
    def forward(
        ...
        c2f = F.interpolate(
            c2f, size=c1.shape[2:], mode="bilinear", align_corners=False
        )
        ...
        c3f = F.interpolate(
            c3f, size=c1.shape[2:], mode="bilinear", align_corners=False
        )
        ...
        c4f = F.interpolate(
            c4f, size=c1.shape[2:], mode="bilinear", align_corners=False
        )
...
class SegFormer(nn.Module):
    def __init__(
        self, model_name: str = "B0", num_classes: int = 19, image_size: int = 224
    ) -> None:
        super().__init__()
        assert (
