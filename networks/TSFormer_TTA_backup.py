...
# MODIFY THE NASEGFORMER TO SUPPORT ADABN NORM INSTEAD
# CONTRIBUTED WARMING UP STEPS TO TRAIN FASTER
class CBA(nn.Module):
    def __init__(
        self, in_planes, out_planes, kernel_size=3, stride=1, padding=1, warm_n=5
    ):
        super(CBA, self).__init__()
        self.conv = nn.Conv2d(
            in_planes, out_planes, kernel_size, stride, padding, bias=False
        )
        self.bn = norm(out_planes, warm_n)
...
class BridgeLayer_4(nn.Module):
    ...
    def forward(self, inputs):
        ...
        m1f = self.mixffn1(tem1, self.img_size // 4, self.img_size // 4).reshape(
            B, -1, C
        )
        m2f = self.mixffn2(tem2, self.img_size // 8, self.img_size // 8).reshape(
            B, -1, C
        )
        m3f = self.mixffn3(tem3, self.img_size // 16, self.img_size // 16).reshape(
            B, -1, C
        )
        m4f = self.mixffn4(tem4, self.img_size // 32, self.img_size // 32).reshape(
            B, -1, C
        )
...
class TSFormer(nn.Module):
    def __init__(
        ...
        heads = [1, 2, 5, 8]
        d_base_feat_size = 8  # 16 for 512 inputsize   7for 224  8 for 256 custom
        ...
        self.conv1 = CBA(in_planes=64, out_planes=256, warm_n=warm_n)
        self.conv2 = CBA(in_planes=128, out_planes=512, warm_n=warm_n)
        self.conv3 = CBA(in_planes=320, out_planes=1024, warm_n=warm_n)
...
if __name__ == "__main__":
    ...
    net = TSFormer(num_classes=num_classes, img_size=img_size, warm_n=5).cuda()
