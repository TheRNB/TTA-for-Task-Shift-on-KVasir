...
class VPTTA_NAFormer:
    def __init__(self, config):
        ...
        # OUR CONTRIBUTION: confidence threshold for sample acceptance
        self.conf_threshold = getattr(config, "conf_threshold")
        # flags
        self.use_tta_aug = getattr(config, "use_tta_aug")
        self.use_conf_filter = getattr(config, "use_conf_filter")
    ...
    def geom_aug(self, prompt_x):
        """
        Test-Time Augmentation
        - identity
        - horizontal flip
        - vertical flip
        then unflip logits and average them
        """
        logits_list = []

        # identity
        logit_orig, *_ = self.model(prompt_x)
        logits_list.append(logit_orig)

        # H flip
        x_h = torch.flip(prompt_x, dims=[3])
        logit_h, *_ = self.model(x_h)
        logit_h = torch.flip(logit_h, dims=[3])
        logits_list.append(logit_h)

        # V flip
        x_v = torch.flip(prompt_x, dims=[2])
        logit_v, *_ = self.model(x_v)
        logit_v = torch.flip(logit_v, dims=[2])
        logits_list.append(logit_v)

        # Soft ensmble
        final_logit = sum(logits_list) / len(logits_list)
        return final_logit
    ...
    def run(self):
        ...
        for batch_idx, (x, y, paths) in enumerate(
            ...
            for _ in range(self.iters):
                ...
                for module in self.model.modules():
                    if isinstance(module, AdaBN):
                        ...
                        count += 1
                loss = bn_loss / count if count > 0 else bn_loss
            ...
            with torch.no_grad():
                ...
                # flag determine if we use identity/H/V flipping
                if self.use_tta_aug:
                    pred_logit = self.geom_aug(prompt_x)
                else:
                    pred_logit, *_ = self.model(prompt_x)

                seg_prob = torch.sigmoid(pred_logit)
                mean_conf = seg_prob.mean().item()

            # Confidence-filtered memory update
            if not self.use_conf_filter:
            ...
            # else:
            # print(f"Skip the memory update... === (conf={mean_conf:.2f} < {self.conf_threshold})")

            # Metrics
            seg_output = seg_prob
            for i in range(seg_output.shape[0]):
                all_dice.append(dice_metric(seg_output[i : i + 1], y[i : i + 1]))
                all_iou.append(iou_metric(seg_output[i : i + 1], y[i : i + 1]))

        print("\nVPTTA-NAFormer TEST RESULTS")
        print(f"Test Dice: {np.mean(all_dice):.2f}%")
        print(f"Test IoU:  {np.mean(all_iou):.2f}%")
...
def main():
    ...
    # OUR CONTRIBUTION: Confidence filtering used for sample acceptance
    parser.add_argument(
        "--conf_threshold",
        type=float,
        default=0.6,
        help="Mean prediction confidence threshold for memory update",
    )

    parser.add_argument(
        "--use_tta_aug",
        action="store_true",
        help="Enable geometric TTA (flip H/V) during inference",
    )

    parser.add_argument(
        "--use_conf_filter",
        action="store_true",
        help="Enable confidence-based sample acceptance",
    )
